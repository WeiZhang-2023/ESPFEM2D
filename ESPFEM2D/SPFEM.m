...
[par,monitor,nCoor,nMat,nAccel,nVel,nStress,nBoundT,nBoundV] ...
    =input_data(iEX);
[par,eNode,nREN,nREs,nRNN,nRNs,nDisp,nPstrain_eq,nF,nFhg]=initializing(par,nCoor);
...
while (ctime<par.totaltime+1.e-9)
    ...
    if (par.hour_glass_alpha>1.e-6)
        nFhg=force_hourglass(par,nRNN,nRNs,nMat,nArea,nCoor,nVel,nF);
    end
    %time integration
    [nVel,nDisp,nCoor]=time_integration(par,nMat,nArea,nBoundT,nBoundV,nVel,nDisp,nCoor,nFint,nFhg);
