...
par.dir_plus='stage2';
par.totaltime=0.6;
par.dtime=1.e-6; %t_c=3.8158e-04
par.hour_glass_alpha=0.1;
par.MN=16; %possible maximum number of related elements
par.vtk=1000;
par.damping=0.;
par.rigid_wall=1; %number of rigid walls
%x0 y0 x1 y1 mu
par.wall=[0.2, 0., 1., 0., 10.]; % n x 5 matrix, where n is the total number of rigid walls
%% Mesh parameters
...
par.alpha_shape=[1,2.5,-1000,1000,-1000,1000]; % n x 6 matrix, where n is the total number of alpha shape treatments
par.remesh_thereshold=20; % if the minimum angle of elements is less than it, the remeshing is conducted
par.max_remesh_thereshold=20; %maximum remesh_thereshold
par.min_remesh_thereshold=5;
...
par.mat_props=[2650,5.84e6,0.3,0.,19.8,0.]; %n x p matrix, where n is the total number of material types
%1 elas 2 DP
par.mat_model=[2];  %n x 1 matrix, where n is the total number of material types
%% Computational mesh
load ans_soil_non_cohesive_soil
par.space=[0.002]; %space for all bodies
% solid, body 1
...
monitor.ID=[102, 102, 102, 102, 102, 102, 102, 102];
